...
\texttt{github.com/microprediction/schur}. Every claim below is checked there
...
and on estimated covariances it does so in about a quarter of markets. Pricing
...
difference is exactly one factor.
...
\paragraph{The trade this represents.} Pricing with $E^\top\Sigma E$ rather than
with $E^\top\Sigma D_0$ costs the exactness of \cref{prop:far}, since the
method is then outside the family. It buys back exactly what
\cref{sec:interior} says is at risk: $E^\top\Sigma E$ is a principal submatrix
of $\Sigma$, hence symmetric positive definite, hence never singular. The
construction is pole-free for the same reason the matched case is, without being
matched. Read this way the choice in \cite{bajo2026} is the conservative one,
and the alternative, substituting the pairing $E^\top\Sigma D_\gamma$ and
sweeping $\gamma$, is what turns the architecture into a bridge with an exact far
end and a fragile interior.
...
Working paper, \texttt{schur.microprediction.org}, 2026.
