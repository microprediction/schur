...
Fix an asset order and a balanced bisection tree over $n$ assets; assume for
notational simplicity that $n$ is a power of two, so sibling blocks have
equal size (in the general case a step-up matrix enters the augmentation
below; see \cite{cotton2024schur}). Given a covariance matrix
$M \succ 0$ and $\gamma \in [0,1]$, the Schur-complementary
allocation $w(\gamma; M)$ is defined recursively
\cite{cotton2024schur}. At a node whose block of the (possibly already
augmented) covariance is
$\left(\begin{smallmatrix} A & B \\ B^\top & D \end{smallmatrix}\right)$,
with $A$ the left child's block, $D$ the right child's, and $B$ the
cross-block between them, each child block is augmented with the
$\gamma$-scaled Schur complement of the other:
\begin{equation}
  ...
  \label{eq:aug}
\end{equation}
where $\operatorname{sym}(X) = \tfrac12(X + X^\top)$ (the damped product is
not exactly symmetric for $0 < \gamma < 1$). The capital allocated to the
node is split between its children in inverse proportion to their
\emph{naive portfolio variances}
\begin{equation}
  ...
  \label{eq:naive}
\end{equation}
i.e.\ the left child receives the fraction
$\nu(D_\gamma) / \big(\nu(A_\gamma) + \nu(D_\gamma)\big)$, and the recursion
continues inside each augmented block, down to singletons. (Nodes whose
children are singletons perform no augmentation --- matching the reference
implementations --- so a two-asset node splits by the inverse of its, already
augmented, diagonal entries.) At $\gamma = 0$
the augmentation is the identity, \eqref{eq:aug} returning $A$ and $D$
unchanged, and the recursion is exactly HRP over the fixed order; as
$\gamma \to 1$ it recovers minimum variance \cite{cotton2024schur}.
