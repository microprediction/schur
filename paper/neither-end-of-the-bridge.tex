...
coupling, so the minimizer is interior. A four-asset example verified in
exact rational arithmetic makes this unconditional. Under natural shape
conditions the optimal coupling is the unique root of
$\tau^2 = -V_0'(\gamma)/G'(\gamma)$ and decreases monotonically in the noise
level: every interior coupling is optimal at exactly one noise level,
minimum variance only as the noise vanishes, hierarchical risk parity only
as it diverges.
\end{abstract}
...
minimizer is interior. An example in the minimal non-trivial configuration
(four assets in two blocks, the smallest universe on which $\gamma$ acts at
all) is verified in exact rational arithmetic. (iv)~Under shape
...
portfolio rather than replicate it. The inter-group fitness must, however,
be a quadratic form evaluated on the \emph{augmented} block, as above: a
diagonal-only $\nu$, used in some HRP variants, never sees the augmentation,
and then no value of $\gamma$ connects the scheme to minimum variance. None
of the results below rely on exactness at $\gamma = 1$.
...
augmentation and fitness routines.} take this route, and it is the
...
dial whose optimal setting is interior, decreasing in the noise, and
characterized by $\tau^2 = -V_0'/G'$. Exact recovery of minimum variance at
full coupling does not change this, and neither does the noise-robustness of
HRP. A walk-forward bridge on Dow constituents, whose minimum near
$\gamma \approx 0.55$ survives Ledoit--Wolf shrinkage of the inputs, is
interactive at \texttt{schur.microprediction.org}.
