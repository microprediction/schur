...
clusters. Under estimation error the best $\gamma$ can be strictly interior
or can stay at full coupling, and we give the local theorem at the
minimum-variance end with exact examples of both.
\end{abstract}
...
transfers, and exact examples show that both an interior optimum and full
coupling occur.
...
quantity in the recipe is computed from $\widehat\Sigma_\tau$. Define the
expected out-of-sample variance and its noiseless profile,
\begin{equation}
  ...
  \label{eq:F}
\end{equation}
We assume $F$ is smooth. It is even in $\tau$.
...
On the tree the hierarchical end reads no cross-block covariance, and the
uncentred cost $F-V_0$ already carries the factor $\gamma$
\cite{cotton2026bridge}. NCO is different. Its outer tier reads
...
end. The cost of moving away from NCO is still of order $\gamma\tau^2$.
...
write $F(\gamma,\tau)=V_0(\gamma)+\tau^2G(\gamma)+O(\tau^4)$. Then the
stationary point of $F(\cdot,\tau)$ near $1$ is
\begin{equation}
  \tilde\gamma(\tau)\;=\;1-\frac{G'(1)}{V_0''(1)}\,\tau^2+O(\tau^4).
  \label{eq:shift}
\end{equation}
If $G'(1)>0$ the optimum over $[0,1]$ is strictly interior for all
sufficiently small $\tau>0$. If $G'(1)<0$ full coupling is a strict local
optimum for all sufficiently small $\tau$. If $G'(1)=0$ the fourth order
decides.
\end{proposition}

\begin{proof}
Normalize $w_\gamma$ to sum to one, so $\ones^\top\dot w=0$ and
...
$\partial_\gamma^2F=V_0''(1)>0$.
\end{proof}

The structure of the bridge does not determine the sign of $G'(1)$. Both
signs occur, and each is strict in the examples below, so each holds on an
open set of covariances and noise laws.
...
$G'(1)\approx0.0573$ and $V_0''(1)\approx0.01347$, so \eqref{eq:shift}
...
\paragraph{Full coupling stays optimal.} Take three clusters, each a knot
and one remaining member that is uncorrelated with everything, with member
variances $1$, $4$ and $8$. Let the knot covariance and its estimates be
\begin{equation*}
...
cross-knot entries, and preserves the gateway model. Here
$V_0''(1)\approx2.3949$ and $G'(1)\approx-0.0058498<0$. In exact arithmetic,
at $\tau=\tfrac1{10}$ and at $\tau=\tfrac1{100}$, $F(\gamma,\tau)>F(1,\tau)$
for $\gamma\in\{0.999,\,0.99,\,0.9,\,0.5,\,0\}$.

Neither end owns the bridge. An interior optimum is not a consequence of the
architecture, and a statement that it is generic needs a noise model.
