...
\begin{equation}
  w_{\gamma\eta}\;\propto\;\sum_C
  \frac{b_C^\top z_C}{z_C^\top Q_C z_C}\,z_C,
  \qquad
  z_C=z\big(Q_C(\gamma),b_C(\gamma),\eta\big),
  \label{eq:square}
\end{equation}
with a cluster whose direction $z_C$ vanishes contributing nothing.
\end{definition}

When $b_C^\top z_C\neq0$ the summand is $v_C/\nu_C$ with
$v_C=z_C/b_C^\top z_C$ the cluster's holding normalized by its companion
vector and $\nu_C=v_C^\top Q_Cv_C$ its variance on the pair, so
\eqref{eq:square} is the inverse-variance budget rule in the form
\eqref{eq:fitness}. The unnormalized form is the one we use, because it has
no singularity. On ordinary positive-definite input a companion vector can
vanish: $\Sigma=\left(\begin{smallmatrix}2&1\\1&1\end{smallmatrix}\right)$
with singleton clusters has $b_{\{1\}}(1)=0$, so $z_{\{1\}}=0$ at $\gamma=1$
and the normalized holding $v_{\{1\}}$ is $0/0$, while
$\Sigma^{-1}\ones=(0,1)^\top$ is perfectly well defined. By
\eqref{eq:blockinv} a vanishing companion means that block of the global
direction is zero, which is what \eqref{eq:square} assigns. The same
convention is used for the flat bridge in \cite{cotton2026nco}.
...
$(\{i\},C\setminus\{i\})$, so $z=Q^{-1}b$, and then
$b^\top z/(z^\top Qz)=1$, so the summand is $Q^{-1}b$ whether or not $b$
vanishes. At $\gamma=1$ the pair is $(\Sigma^c_C,b_C)$ and
...
that $\beta=(\sum_i\sigma_i^{-1})^2-\alpha$. With $h_i=\kappa\sigma_i s_{-i}$
the long-only frontier is
\begin{equation}
  \eta_+=\min\big(\{1\}\cup\{1/h_i : h_i>1\}\big),
  \label{eq:equifrontier}
\end{equation}
which is $\min_i 1/h_i$ when some asset is shorted inside the square and $1$
otherwise, including every $\rho\le0$, where $\kappa\le0$ and nothing is
ever shorted.
\end{proposition}
...
\eqref{eq:frontier} with $b=\ones$ and $s_i=1-h_i$: the numerator of asset
$i$ crosses zero inside $[0,1]$ exactly when $h_i>1$, at $\eta=1/h_i$.
\end{proof}
